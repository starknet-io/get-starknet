...
export class MetaMaskSnapWallet implements IStarknetWindowObject {
  ...
  async enable(
    ...
    const response = await this.metamaskProvider.request<RequestSnapResponse>({
      ...
      params: {
        [this.snapId]: { version: MetaMaskSnapWallet.snapVersion },
      },
    })
    if (!response) {
      throw new Error("The snap was not found in the response")
    }
    ...
    if (snapResponse && snapResponse.enabled) {
      ...
      const accountResponse = await this.snap.recoverDefaultAccount(
        this.chainId,
      )
