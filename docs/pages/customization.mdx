...
Use the [`useConnect`](/modal/useConnect) hook to get the currently connected wallet.
With this information, you can create a custom button to open the modal.

### Show wallet list

Use the [`WalletList`](/modal/WalletList) and [`SelectedWallet`](/modal/SelectedWallet) components together to create an UI that lets the user select a wallet.
